...
Load this skill when copy already exists and the reader wants it improved or refreshed — not written from scratch (that's **copywriting**). Run the Seven Sweeps in order, present findings, recommend specific edits, and let the author keep final say. For high-stakes copy, add the Expert Panel scoring gate. For quick jobs, use the Quick-Pass checks. End with the Self-check.

## Freedom Dial: HIGH (judgment work)

Copy editing has many correct answers — this is judgment work, so the body below is **principles and guardrails, not a rigid script**. The *sequence* of sweeps is fixed (run them in order), but inside each sweep use your judgment about what to flag and how to fix it. Do not mechanically apply every rule; apply the ones that serve this reader and this brand voice. The one hard rule: never rewrite the core message — enhance it.

## North Star objective

Make existing copy clearer, more persuasive, and more trustworthy **without changing its core message or the author's voice** — every edit must have a stated reason, and a full clean sweep finds nothing left to fix.
...
Before delivering edits, confirm:

- [ ] **Core message preserved** — the edited copy says the same thing, only better; the author's voice survives.
- [ ] **Seven Sweeps run in order** — Clarity → Voice/Tone → So What → Prove It → Specificity → Heightened Emotion → Zero Risk, with loop-backs after each.
- [ ] **Every edit has a stated reason** — no silent changes; each fix names the issue it solves.
- [ ] **Claims are honest** — Prove It and Specificity didn't invent numbers, testimonials, or proof that doesn't exist (for pet-relocation copy, never fabricate a customs/airline figure or guarantee — flag it for the author to source).
- [ ] **Final clean sweep** — a last full pass surfaces no new issues.
- [ ] **Right tool** — this is a refresh/edit job, not a from-scratch write (else defer to copywriting).

---

## Known gaps

- **Does not write from scratch.** If the existing copy is fundamentally broken (Expert Panel scores 1–2), this skill can flag it but the rebuild belongs to **copywriting**, not here.
- **Cannot source proof.** Sweep 4 (Prove It) and Sweep 5 (Specificity) identify missing evidence and vague numbers, but the skill cannot verify or supply real statistics, testimonials, or claims — it hands those back to the author. For pet-relocation copy this matters: confiscation, customs, and airline figures must be author-supplied and verified, never generated to fill a gap.
- **No live brand voice unless provided.** Voice/Tone sweep relies on `.agents/product-marketing.md` (or its legacy filenames); with no context file it edits toward generic best practice and may drift from house voice.
- **Not a layout/CSS tool.** It edits words; structural and strategic page changes defer to **cro**.

...
Run `/blog cluster` and pick **plan** or **execute** (see Commands). Plan turns
a seed keyword into a hub-and-spoke `cluster-plan.json` + map; execute writes
every post by orchestrating `blog-write`. Heavy detail lives in the three
reference files — load them only on the condition stated beside each step.

## North Star objective

From **one seed keyword**, produce a complete, internally consistent, fully
interlinked content ecosystem — one pillar plus 5 to 15 spokes — with **zero
keyword cannibalization** and **every spoke earning at least 3 incoming links**.
The cluster must read as one coherent structure, not a pile of posts.

## Freedom Dial — MIXED (plan = high · execute = low)

State both phases and keep them separate:

- **Plan Phase = HIGH freedom (judgment work).** Keyword grouping, intent
  classification, and cluster architecture have many valid answers. Follow the
  principles in the reference files; do not treat the steps as a rigid script.
  Use SERP overlap and topical sense, then reason.
- **Execute Phase = LOW freedom (precision work).** Once a plan exists, execution
  is mechanical: fixed order, fixed context block, fixed link-injection and
  scorecard rules. Variance here = broken links and cannibalization. Follow the
...
- **Auto-executing after planning.** Always present the plan and wait for explicit
  user approval (Plan Step 6). Never write 15 posts the user has not seen the shape of.
- **Treating the Plan Phase as a checklist.** Keyword grouping is judgment work;
  mechanically dumping every expanded keyword into one post per intent produces a
  flat, cannibalizing cluster. Use SERP overlap to merge and split.
- **Skipping FLOW evidence propagation in the cluster context.** Omitting the
  evidence-triple directive lets individual spokes silently drop citation discipline
  across 5 to 15 posts at once.
- **JavaScript in `cluster-map.html`.** No `<script>`, no `on*` handlers, no external
  `src`. Hover is CSS-only and every SVG label is HTML-escaped. A map with script is a fail.
- **Aborting the whole cluster on one post failure.** Log, skip, continue; mark the
  gap in the scorecard (Execute Step 6).
- **Silently overriding the user's strategy table.** On `--from strategy`, flag SERP
  conflicts; do not rewrite their strategic intent.

## Real examples

**Plan input:** `/blog cluster plan dubai pet relocation`
**Plan output (abridged):** pillar `P` = "Dubai Pet Relocation: The Complete 2026 Guide"
(template `pillar-page`, 3,000 words); Cluster A (informational): A1 "How to Export a
Dog from Dubai", A2 "UAE Pet Export Health Certificate Explained"; Cluster B
(transactional): B1 "Dubai Pet Relocation Cost Breakdown", B2 "Best Pet Relocation
Companies in Dubai". `cluster-plan.json` reports `total_posts: 5, total_interlinks: 12`.
A `cluster-map.html` renders the pillar node with two color-coded cluster groups.

**Execute input:** `/blog cluster execute cluster-dubai-pet-relocation/cluster-plan.json`
**Execute behavior:** writes `pillar-dubai-pet-relocation.md` first, then spokes in
volume order; each spoke's cluster context block carries the FLOW evidence directive
and `[INTERNAL-LINK: ...]` placeholders, which are resolved to real markdown links as
later posts land. Ends with `cluster-scorecard.md` showing per-post quality scores,
the cohesion score, and an orphan/cannibalization audit.

## Self-check validation

- [ ] Plan Phase reasoned over SERP overlap (HIGH freedom), not a rote keyword dump.
- [ ] Plan presented to the user; execution waited for explicit approval.
- [ ] `cluster-map.html` contains zero `<script>`/`on*`/external `src`; all SVG labels escaped.
- [ ] Cluster context block for every post carries the FLOW evidence-triple directive.
- [ ] Execute Phase followed the fixed order, link-injection, and scorecard steps (LOW freedom).
- [ ] Every spoke has at least 3 incoming links; no two posts share a primary keyword.
- [ ] One post failing did not abort the cluster; the gap is logged in the scorecard.

## Known gaps

- **No real search-volume data.** Volume estimates are relative (high/medium/low)
  inferred from SERP signals, not API figures. For precise volumes the user must run
  the `seo-dataforseo` sibling or an external tool (Ahrefs, SEMrush).
- **SERP overlap is reasoning-based.** Without WebSearch the skill falls back to
  Claude's reasoning, reducing clustering accuracy; this is noted in the scorecard.
- **No publishing or scheduling.** blog-cluster writes files only; use `blog-calendar`
  to schedule and the platform's own tooling to publish.
- **Hero images are optional and best-effort.** If `nanobanana-mcp` is absent, images
  are skipped silently after one warning; the skill does not retry or source stock art.
- **Quality gates warn, they don't auto-fix.** Intent/template/link-diversity gates
  surface problems in the scorecard but do not rewrite the plan to resolve them.

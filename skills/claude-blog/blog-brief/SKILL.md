...
rankable post without re-researching: every keyword, statistic, outline section,
visual, and link is decided and sourced up front. A good brief is **research
done once, reused everywhere** — the writer makes prose decisions, never strategy
decisions.

## Freedom Dial: MIXED (plan high → output low)

- **Planning phase (Steps 1-4) — HIGH freedom.** Choosing the angle, template,
  keywords, competitive gaps, and information-gain hooks is judgement work.
  Reason from search intent and what actually ranks; do not follow a rigid recipe.
- **Output phase (Step 5-6) — LOW freedom.** The brief's structure is a fixed
...
- **Brief with no sources.** Every statistic row must carry a real source + URL +
  year. A stat with `[source]` left as a placeholder is a failed brief — the
  writer will either drop it or invent one.
- **Skipping competitive analysis.** Recommending a 1,200-word listicle when the
  top 5 results are 3,000-word guides loses before writing starts. Always check
  what ranks (Step 3) before setting word count and template.
- **Generic information-gain hooks.** "[ORIGINAL DATA]: add a survey" is useless.
  Name the specific data only this author can produce (e.g. "anonymised
  confiscation-rate data from 200 Dubai-to-UK pet moves we handled in 2025").
- **Reordering or renaming output sections.** `/blog write` parses the schema in
  Step 5. Treat that block as a contract, not a suggestion.
- **Swapping terms.** Hold one term per concept: "primary keyword" (not "main
  keyword"/"head term"), "capsule" (not "snippet"/"extract"), "template" (not
  "format"/"type") across the whole brief.

## Real examples

**Input:** `/blog brief moving a dog from Dubai to the UK`

**Brief decisions produced (abridged):**
- **Template:** `how-to-guide` — search intent is procedural; top 4 results are
  step-by-step guides averaging 2,400 words.
- **Primary keyword:** "moving a dog from Dubai to the UK"; **secondary:**
  "UK pet import rules", "rabies titer test UAE", "pet relocation cost Dubai".
- **Questions:** "Does my dog need to quarantine entering the UK?",
  "How long does the rabies blood test take?".
- **Information gain — [ORIGINAL DATA]:** "Average door-to-door cost + timeline
  from 50 real Dubai→Heathrow moves we shipped in 2025."
- **Competitive gap:** competitors omit the 21-day post-vaccination wait math; we
  give a dated countdown calendar.
- **Stat row:** "Defra requires a rabies antibody titre of ≥0.5 IU/ml | GOV.UK
  Bringing your pet dog to Great Britain | 2025 | H2: Blood test".

## Self-check validation

- [ ] Every statistic row has a real value + source + URL + year (no placeholders).
- [ ] Template recommendation is justified against what currently ranks (Step 3).
- [ ] Word count / H2 count are set from the competitive average, not a default.
- [ ] Information-gain hooks name author-specific data, not generic advice.
- [ ] Output uses the Step 5 schema verbatim — all sections present and in order.
- [ ] Terms held constant (primary keyword / capsule / template) throughout.
- [ ] Brief saved to `briefs/[slug]-brief.md` and confirmed ready for `/blog write`.

## Known gaps

- **Search volume is best-effort.** WebSearch does not return reliable monthly
  volume; the "estimated monthly search volume" field is often blank or a guess.
  No keyword-tool API is wired in.
- **Reference files are not all present.** `references/content-templates.md`,
  `references/distribution-playbook.md`, and `references/internal-linking.md` are
  cited but may not yet exist in this folder; the cross-skill refs under
  `skills/blog/references/` live in the orchestrator, not here. If a referenced
  file is missing, fall back to the inline tables in this skill.
- **Does not write the post.** This skill plans only; drafting is `/blog write`.
- **No live SERP scraping.** Competitive analysis relies on WebSearch result
  snippets, not full-page crawls, so heading-structure counts are estimates.
